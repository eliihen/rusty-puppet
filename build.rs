...
fn extract_archive(file: &mut File, outpath: &Path) {
    ...
    for i in 0..archive.len() {
        ...
        if (&*file.name()).ends_with('/') {
            println!("File {} extracted to \"{}\"", file.name(), outfile_name.display());
            fs::create_dir_all(&outfile_name).unwrap();
        } else {
            println!("File {} extracted to \"{}\" ({} bytes)", file.name(), outfile_name.display(), file.size());
        ...
        #[cfg(unix)]
        {
            ...
            if let Some(mode) = file.unix_mode() {
                println!("Setting permissions {} on \"{}\"", mode, outfile_name.display());
                fs::set_permissions(
                    &outfile_name,
...
fn download(dest_path: &Path) {
    ...
    if !target_dir.exists() {
        ...
        let mut result = reqwest::get(DOWNLOAD_URL.as_str())
            .expect("Failed to downoad chrome binary");
